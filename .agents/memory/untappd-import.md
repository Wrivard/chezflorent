...
- `FOOD_SLUGS` (ardoise, encas, salades, pizzas, hoagies) drives the HOMEPAGE `Menu` component tabs, ordered per the array — homepage shows kitchen food only, ardoise leads as default tab.
- MENU PAGE split: `menu = categories in MENU_SLUGS` renders as ONE unified tab bar ("La cuisine", sticky cross-fade photo panel); `bar = categories NOT in FIXED_MENU_SLUGS` (imported Untappd drinks) renders in a separate photo-less "Le bar" board (`showPhoto={false}`, two-column grid).
  ...
  - **`alcools` is hidden from the main menu:** it is in `FIXED_MENU_SLUGS` but NOT in `MENU_SLUGS`, so it shows in NEITHER board (removed from La cuisine per user request, and excluded from Le bar because Le bar is Untappd-only). It still lives in the DB and is protected from import deletion. The bar filter keys off `FIXED_MENU_SLUGS` (not `MENU_SLUGS`) precisely so this hidden-but-fixed category doesn't leak into Le bar.

## Admin CMS Menu editor scope (drinks are Untappd-only)
- The CMS `MenuEditor` manages ONLY the fixed site-owned FOOD categories. It filters visible categories to `FIXED_MENU_SLUGS` MINUS `HIDDEN_EDITOR_SLUGS` (`alcools`) — so ALL Untappd-imported drink categories (alcool, gin, rhum, whisky, wines, en-futs, cocktails, etc.) AND the fixed-but-hidden `alcools` are hidden from the admin.
- **Why:** the owner manages the entire bar through the Untappd app, and any CMS edit to a drink category is wiped on the next import anyway (they're non-protected). Showing them was a confusing footgun.
- The "Ajouter une catégorie" (AddCategory) feature was REMOVED from `MenuEditor`: a hand-added category gets a non-fixed slug, so it's both hidden by the filter and deleted on the next Untappd import. Do not re-add it without first giving categories a persisted site-owned/imported ownership flag.
- **Drift risk:** the fixed-slug list now lives in THREE places — `App.tsx` `FIXED_MENU_SLUGS`, importer `PROTECTED_SLUGS`, and `MenuEditor` `FIXED_MENU_SLUGS`. Keep all three in lockstep (a shared workspace lib would be the real fix).
