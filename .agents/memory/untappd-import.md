...
- `PROTECTED_SLUGS` is the allowlist of FIXED printed-menu categories that the importer must NEVER touch. It MUST stay in sync with `FIXED_MENU_SLUGS` in the frontend `App.tsx` (currently: encas, salades, pizzas, hoagies, desserts, cafes-thes, alcools, extras). EVERY other category is considered importer-owned (Untappd drinks) and is deleted+reinserted wholesale each run (`notInArray(slug, PROTECTED_SLUGS)`).
...
- Two distinct frontend constants (do not conflate): `FIXED_MENU_SLUGS` = all 8 site-owned fixed categories (mirrors importer `PROTECTED_SLUGS`); `MENU_SLUGS` = the subset actually SHOWN in the main "La cuisine" tab bar.
